<xsl:stylesheet
  ...
  <xsl:template name="OuterTemplate">
    ...
    <xsl:variable name="selectedItemStyle" select="/dsQueryResponse/Rows/Row[1]/@Style" />
    <!-- <xsl:value-of select="$selectedItemStyle" />-->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:call-template name="OuterTemplate.Body">
            ...
            <xsl:with-param name="selectedItemStyle" select="$selectedItemStyle" />
          </xsl:call-template>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template name="OuterTemplate.Body">
    ...
    <xsl:variable name="COMBeginColumn">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$BeginColumn1"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <!--<xsl:variable name="BeginColumn" select="concat($COMBeginColumn, $cbq_columnwidth, $BeginColumn2)" />-->
    <xsl:variable name="BeginColumn" select="$COMBeginColumn" />
    ...
    <xsl:for-each select="$Rows">
      ...
      <xsl:if test="($CurPosition &gt;= $FirstRow and $CurPosition &lt;= $LastRow)">
          ...
          <xsl:when test="$StartNewGroup and $StartNewColumn">
            ...
            <xsl:choose>
              <xsl:when test="$CurPosition = $FirstRow">
                ...
                <xsl:variable name="createGroupUl">
                  <xsl:call-template name="BeginGroupUl">
                    <xsl:with-param name="CurrentRow" select="." />
                  </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$createGroupUl" disable-output-escaping="yes"/>
                <xsl:call-template name="OuterTemplate.CallHeaderTemplate">
                  <xsl:with-param name="SelectedItemStyle" select="@Style" />
                </xsl:call-template>
              ...
              <xsl:otherwise>
                ...
                <xsl:variable name="endGroupUl">
                  <xsl:call-template name="EndGroupUl">
                    <xsl:with-param name="CurrentRow" select="." />
                  </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="$endGroupUl" disable-output-escaping="yes"/>
                <xsl:variable name="createGroupUl">
                  <xsl:call-template name="BeginGroupUl">
                    <xsl:with-param name="CurrentRow" select="." />
                  </xsl:call-template>
        ...
        <xsl:if test="$CurPosition = $LastRow">
          ...
          <xsl:value-of disable-output-escaping="yes" select="$COMEndColumn" />
        </xsl:if>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="OuterTemplate.CallHeaderTemplate">
    ...
    <xsl:choose>
      <xsl:when test="$SelectedItemStyle = 'Alerts'">
        <xsl:value-of disable-output-escaping="yes" select="string('&lt;ul&gt;')" />
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="BeginGroupUl">
    <xsl:param name="CurrentRow" />

    <xsl:if test="$CurrentRow/@Style = 'Alerts'">
      <xsl:choose>
        <xsl:when test="$CurrentRow/@PSO_Criticality = '1 - Critical'">
          <xsl:value-of select="string('&lt;ul class=&quot;default / pso-alerts / alerts-critical&quot;&gt;')" disable-output-escaping="yes" />
        </xsl:when>
        <xsl:when test="$CurrentRow/@PSO_Criticality = '2 - Watch'">
          <xsl:value-of select="string('&lt;ul class=&quot;default / pso-alerts / alerts-watch&quot;&gt;')" disable-output-escaping="yes" />
        </xsl:when>
        <xsl:when test="$CurrentRow/@PSO_Criticality = '3 - Clear'">
          <xsl:value-of select="string('&lt;ul class=&quot;default / pso-alerts / alerts-clear&quot;&gt;')" disable-output-escaping="yes" />
        </xsl:when>
      </xsl:choose>
    </xsl:if>
  ...
  <xsl:template name="EndGroupUl">
    <xsl:param name="CurrentRow" />

    <xsl:value-of select="string('&lt;/ul&gt;')" disable-output-escaping="yes"/>
  ...
  <xsl:template name="OuterTemplate.CallItemTemplate">
    ...
    <xsl:value-of disable-output-escaping="yes" select="$COMBeginListItem" />
    ...
    <xsl:variable name="COMEndListItem">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$EndListItem"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>


    <xsl:value-of disable-output-escaping="yes" select="$COMEndListItem" />
  </xsl:template>

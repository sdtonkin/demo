...
/***/ (function(module, exports) {
	...
	angular.module('compassionIntranet').service('photoService', ['$http', '$q', 'COM_CONFIG', 'storage', 'common', function ($http, $q, COM_CONFIG, storage, common) {
	    ...
	    function getPhotos() {
	        var defer = $q.defer();
	        var web = new $pnp.Web(COM_CONFIG.rootWeb);
	        web.lists.getByTitle(COM_CONFIG.lists.missionPhotos).items.select('COM_PhotoCaption', 'EncodedAbsUrl').orderBy('Created', false).get().then(function (data) {
	            var photos = [];
	            for (var i = 0; i < data.length; i++) {
	                var p = data[i];
	                var photo = {};

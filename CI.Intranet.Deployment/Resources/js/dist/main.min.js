...
/***/ (function(module, exports) {
	...
	angular.module('compassionIntranet').service('bookmarkService', ['$http', '$q', 'COM_CONFIG', 'storage', 'common', function ($http, $q, COM_CONFIG, storage, common) {
	    ...
	    function getUserBookmarkItemsByName(userName) {
	        ...
	        if (!local.isExpired) defer.resolve(local);else {
	            var web = new $pnp.Web(COM_CONFIG.rootWeb);
	            web.lists.getByTitle(COM_CONFIG.lists.userBookmarks).items.select('COM_ToolbarUser/SipAddress', 'Title', 'COM_BookmarkUrl', 'COM_ListSortOrder').filter("COM_ToolbarUser/SipAddress eq '" + userName + "'").expand('COM_ToolbarUser').get().then(function (data) {

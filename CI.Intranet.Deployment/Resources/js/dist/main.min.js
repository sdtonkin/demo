...
/***/ (function(module, exports) {
	...
	angular.module('compassionIntranet').service('appService', ['$http', '$q', 'COM_CONFIG', 'storage', 'common', function ($http, $q, COM_CONFIG, storage, common) {
	    ...
	    function getUserAppItemsByName(userName) {
	        ...
	        if (!local.isExpired) defer.resolve(local);else {
	            var web = new $pnp.Web(COM_CONFIG.rootWeb);
	            web.lists.getByTitle(COM_CONFIG.lists.userApps).items.select('COM_ToolbarUser/SipAddress', 'Title', 'COM_UserToolbarId', 'COM_ListSortOrder').filter("COM_ToolbarUser/SipAddress eq '" + userName + "'").expand('COM_ToolbarUser').get().then(function (data) {
	                var promises = new Array();
	                for (var i = 0; data.length > i; i++) {
	                    var p = getUserApp(data[i]);
	                    promises.push(p);
